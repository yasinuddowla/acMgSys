...
<div class="io-form" style="width:70%;margin:0px auto">
    <form action="view_monthly_deposit.php" class="search-form" method="GET">
		<p align="center" style="color: #F00; font-size: 1.2em;">Show Monthly Deposit</p>
...
<?php
$deposits=mysqli_query($connection,"SELECT  * FROM `deposit` ");
...
<?php
	...
	if($a_id=='all'){
		...
		$query="SELECT * FROM `deposit` WHERE date LIKE '{$search_time}%' AND (purpose='{$pps}') and (account_id='{$a_id}') order by date asc";
...
$th='<table class="all-table full-broad report">';
	...
	$pr=6; //no of  rows on a page
		...
		<tr>
			...
			<td>
				<a href="view_daily_deposit.php?purpose=<?php echo $pps?>&account_id=<?php echo $_GET['account_id']?>&from=<?php echo $row['date']?>&to=<?php echo $row['date']?>&submit=Search">
					<?php echo  format_date($row['date'])?>
				</a>
			</td>
			<td>
				<a href="view_monthly_deposit.php?purpose=<?php echo $row['purpose']?>&account_id=<?php echo $_GET['account_id']?>&year=<?php echo $year?>&month=<?php echo intval($month)?>&submit=Search">
