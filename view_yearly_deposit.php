...
<div class="io-form" style="width:70%;margin:0px auto">
    <form action="view_yearly_deposit.php" class="search-form" method="GET">
		<p align="center" style="color: #F00; font-size: 1.2em;">Show Yearly Deposit</p>
...
<?php
	if(isset($_GET['submit'])) { 
		...
		$n1=mysqli_num_rows($deposits);
		
		if(!$n1){?>
			<div id="search_result">
		No Deposit for<br><b><i><?php echo $account_name.'[ July, '.$year.' - June, '. ($year+1).']'?></i></b><br>
...
<table class="all-table min-broad  y-report report">
	<caption class="caption">Deposit Information of <br><b> <?php echo $account_name.'[ July, '.$year.' -
	...
	for($i=7;;){
		...
		$query="select sum(total) as total from `deposit` where  date like '{$search_time}%'  and (purpose='{$pps}') and (account_id='{$a_id}')";
		...
		$deposits=mysqli_fetch_assoc($deposits);
			...
			$monthly_total+=$deposits['total'];
	?>
		<tr>
			<td><?php echo $count++?></td>
			<td>
				<a href="view_monthly_deposit.php?purpose=<?php echo $_GET['purpose']?>&account_id=<?php echo $_GET['account_id']?>&year=<?php echo $year?>&month=<?php echo intval($month)?>&submit=Search">
