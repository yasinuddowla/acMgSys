...
?>
 ...
 <?php
admin_nav();
$purposes=mysqli_query($connection,"SELECT  * FROM `purposes` where type='Both' or type='Deposit'");
$accounts=mysqli_query($connection,"SELECT  * FROM `accounts` ");

 ?> 
<div class="io-form" style="width:70%;margin:0px auto">
    <form action="view_daily_deposit.php" class="search-form" method="GET">
	<p align="center" style="color: #F00; font-size: 1.2em;">Show Daily Deposit</p>
	<p>
...
<?php
	if(isset($_GET['purpose'])) { 
		...
		$query="SELECT * FROM `deposit` WHERE date BETWEEN '{$s1}' and '{$s2}' AND (purpose='{$pps}') and (account_id='{$a_id}') order by date asc";
			

		$deposits=mysqli_query($connection,$query);
		$num_rows=mysqli_num_rows($deposits);
		...
		if(!$num_rows){?>
			<div id="search_result">
				No Deposit for <b><?php echo $account_name?></b><br>on <b><?php echo '['.$duration.']'?></b><br>[Purpose: <b><?php echo ucfirst($purpose); ?>]</b>
			</div>
			<br>
			<br>
		<?php 
...
$th='<table class="all-table full-broad report">';
	$tc='<caption class="caption">Deposit Information for <b>'.$account_name.'</b><br>On <b> ['.
	...
	$pr=6; //no of  rows on a page
		$pages=ceil($num_rows/$pr);
			...
			$row=mysqli_fetch_assoc($deposits);
			$total+=$row['total'];
			$paid+=$row['paid'];
			$due+=$row['due'];
			?>
		<tr>
			...
			<td>
				<a href="preview_voucher.php?type=Deposit&q=<?php echo $row['id']?>&submit=Search">
			<?php echo $row['id']?>
			</a>
			...
			<td>
				<a href="view_daily_deposit.php?from=<?php echo $row['date']?>&to=<?php echo $row['date']?>&purpose=<?php echo $_GET['purpose']?>&account_id=<?php echo $_GET['account_id']?>&submit=Search">
					<?php echo format_date($row['date'])?>
				</a>
			</td>
			<td>
				<a href="view_daily_deposit.php?from=<?php echo $s1?>&to=<?php echo $s2?>&purpose=<?php echo $_GET['purpose']?>&account_id=<?php echo $_GET['account_id']?>&submit=Search">
