...
$purposes=mysqli_query($connection,"SELECT  * FROM `purposes` where type='both' or type='deposit'");
 ?> 
    ...
    <form action="view_monthly_purpose_deposit.php" id="select_form" method="GET">
	<p align="center" style="color: #F00; font-size: 1.2em;">Show Monthly Purpose Based Deposits</p>
	<p>
...
<?php
	if(isset($_GET['submit'])) { 
		...
		$query="select * from `deposit` where date like '{$search_time}%' and purpose='{$purpose}'";
		$deposits=mysqli_query($connection,$query);
		$num_rows=mysqli_num_rows($deposits);
		
		if(!$num_rows){?>
			<div id="search_result">
								No Deposits in <b><?php echo $purpose.' ['.get_month_name($month).', '.$year.' ]'?></b>
			</div>
			<br>
			<br>
		<?php 
...
<table class="all-table full-broad">
		<caption class="caption">Deposit Information for <b>  <?php echo $purpose.' ['.get_month_name($month).', '.$year.' ]'?> </b></caption>
	<tr>
		<th width="5%">#</th>
		<th width="10%">ID</th>
	...
	?>
		<tr>
			...
			<td>
				<a href="view_daily_deposit.php?year=<?php echo $year?>&month=<?php echo intval($month)?>&day=<?php echo intval($day)?>&submit=Search">
					<?php echo $row['date']?>
				</a>
			</td>
